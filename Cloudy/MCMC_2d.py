...
hdu=fits.open(f'Files_n_figures/component_III_nH_Z_col_density_param.fits')
data=Table(hdu[1].data)

hdu_OVI=fits.open(f'Files_n_figures/col_den_OVI_extrapolated.fits')
data_OVI=Table(hdu_OVI[1].data)


log_nH=data['log_nH']
log_Z=data['log_Z']

log_nH_OVI=data_OVI['log_nH']
log_Z_OVI=data_OVI['log_Z']

mask=logical_and(log_nH==nH,log_Z==Z)
mask_OVI=logical_and(log_nH_OVI==nH,log_Z_OVI==Z)

col_den_OVI=data_OVI[mask_OVI][0][2]

model_col_den=[log10(data[mask][i][0]) for i in ions]

err=0.1

noise=[abs(random.normal(0,err,1)[0]) for i in ions]

observations={ions[i]:[model_col_den[i]+noise[i],0.1] for i in range(len(ions))}

print(col_den_OVI)
print([model_col_den[i] for i in range(len(ions))])
print(observations)
...
for i in observations.keys():
    ...
    with open(f'Files_n_figures/{i}_quintic.pkl','rb') as pickle_file:
...
flat_samples_inc_OVI=start_MCMC_samples(ions_to_use2,nsteps=nsteps,guess=[nH,0.1,Z,0.1])
...
corner.corner(flat_samples_inc_OVI, labels=labels, quantiles=[0.16, 0.5, 0.84],show_titles=True, title_kwargs={"fontsize": 15},verbose=True,plot_contours=False)
